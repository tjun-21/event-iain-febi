        ...
        <div class="card">
            <div class="card-header">
                <h5 class="card-title mb-0">
                    ...
                    Form Tambah Requirement
                </h5>
            </div>
            <div class="card-body">
                <form action="{{ route('event-requirements.store', $event->id) }}" method="POST">
                    @csrf
    ...
    $(document).ready(function() {
        ...
        setInterval(function() {
            ...
            if (content && content !== '<p><br></p>') {
                localStorage.setItem('requirement_draft_{{ $event->id }}', content);
                console.log('Draft saved to localStorage');
            }
        }, 30000);

        // Load draft on page load
        var draft = localStorage.getItem('requirement_draft_{{ $event->id }}');
        ...
        $('form').on('submit', function() {
            localStorage.removeItem('requirement_draft_{{ $event->id }}');
        });

        // Manual clear draft button (optional)
